...
class GeminiProxy(BaseProvider):
    ...
    async def forward_to_gemini(self, request: Request, path: str, body: bytes) -> Any:
        ...
        # Parse query parameters and ensure API key is present
                ...
                if role == "user":
                    ...
                    if text and self.omnara_client:
                        ...
                        if not session.omnara_agent_instance_id:
                            ...
                            session.omnara_agent_instance_id = response.agent_instance_id
                        else:
                            # Subsequent message
        ...
        try:
            ...
            if full_text:
                ...
                if self.omnara_client and session.omnara_agent_instance_id:
                    logger.info(f"Sending streamed response to Omnara: {full_text[:100]}...")
    ...
    async def setup(self):
        ...
        if self.config.omnara_api_key:
            self.omnara_client = OmnaraClient(
                api_key=self.config.omnara_api_key,
                base_url=self.config.omnara_base_url
            )
...
def main():
    ...
    if config.auto_launch:
        ...
        print(f"💡 Just start typing to chat with Gemini!")
        print(f"\n📝 Note: If Gemini CLI asks for authentication:")
        print(f"   • Follow the browser prompts to sign in with Google")
        print(f"   • Free tier is available!")
        print(f"   • Or set GEMINI_API_KEY environment variable")
    else:
